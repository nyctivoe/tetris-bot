...
class TetrisFormerV4(nn.Module):
    def __init__(self, embed_dim=192, num_heads=6, depth=4,
        ...
        self.queue_pos_embed = nn.Parameter(torch.randn(1, QUEUE_SLOTS + 2, embed_dim))
        ...
        self.register_buffer('_grid_pos_embed', None, persistent=False)
...
def _compute_rollout_reward(board_after: np.ndarray,
    ...
    if _safe_float(clear_stats.get("b2b_bonus", 0.0)) > 0:
        b2b_cont = 1.0
    elif clear_stats.get("b2b_chain", 0) > prev_b2b:
        # B2B chain increased (qualifying clear maintained the chain)
        # but b2b_bonus is 0 for chain values 1-2.
        b2b_cont = 1.0
...
class SmartRolloutRankDataset(IterableDataset):
    ...
    def __init__(self,
        ...
        self.cache_dir = cache_dir or os.path.join(_ROOT, DEFAULT_CACHE_DIR_NAME)
    ...
    def __iter__(self):
        ...
        for entry in my_entries:
            ...
            try:
                ...
                print(f"[Worker {worker_id}] Skipping corrupted cache {cache_path}: {e}")
                continue
            ...
            if cache_version != CACHE_VERSION:
                raise RuntimeError(
                    ...
                    f"found v{cache_version or 'missing'}. Rerun `uv run python -m tetrisformer.preparse_games`."
                )
...
def main():
    ...
    from tetrisformer.fileParsing import DATA_PATH as _DEFAULT_DATA, INDEX_PATH as _DEFAULT_INDEX
    parser.add_argument("--data-path",  default=_DEFAULT_DATA)
    parser.add_argument("--index-path", default=_DEFAULT_INDEX)
    parser.add_argument("--cache-dir",  default=os.path.join(_ROOT, DEFAULT_CACHE_DIR_NAME))
    parser.add_argument("--save-dir",   default=os.path.join(_ROOT, "checkpoints"))
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if args.val_batches <= 0:
            master_print("[WARN] --val-batches must be > 0 for IterableDataset; skipping validation.")

        with torch.no_grad():
            while v_batches < args.val_batches:
